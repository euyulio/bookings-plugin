...
final class Booking extends Component {
	...
	public function __construct( $args = [] ) {
		...
		if ( is_admin() ) {
			...
			// Alter meta boxes.
			add_filter( 'hivepress/v1/meta_boxes/booking_attribute_search', [ $this, 'alter_booking_attribute_meta_box' ], 100 );
			...
			$this->attributes = array_merge(
				...
				[
					'booking_import_url' => [
						...
						'edit_field' => [
							'label'       => esc_html__( 'Booking Import URL', 'hivepress-bookings' ),
							'description' => esc_html__( 'Set the external ICS calendar URL to sync the availability.', 'hivepress-bookings' ),
							'type'        => 'url',
							'_order'      => 270,
						],
					],
				...
				if ( $user && $vendor ) {
					...
					$email_args = [
						'tokens' => [
							...
							'booking_start' => date('m/d/Y H:i', $booking->get_start_time()),
	...
	public function schedule_booking_import( $id, $object ) {
		...
		if ( $object->get_booking_import_url() ) {
	...
	public function import_bookings( $model, $id ) {
		...
		if ( ! $listing || ! $listing->get_booking_import_url() ) {
			hivepress()->scheduler->remove_action( $action, $action_args );
		...
		// Create calendar.
		try {
			$calendar = @new ICal( $listing->get_booking_import_url() );
	...
	/**
	 * Alters booking attribute meta box.
	 *
	 * @param array $meta_box Meta box arguments.
	 * @return array
	 * @todo Delete when fixed in the core.
	 */
	public function alter_booking_attribute_meta_box( $meta_box ) {
		$meta_box['fields'] = [];
	...
	public function alter_booking_make_form( $form_args, $form ) {
		...
		if ( $booking ) {
			...
			if ( $listing ) {
				...
				if ( $this->is_time_enabled( $listing ) ) {
					...
					if ( is_null( $disabled_dates ) ) {
						...
						foreach ( $bookings as $booking ) {
							...
							if ( ! $is_daily && $booking->get_status() !== 'private' && ( ! $prev_booking || $prev_booking->get_end_time() < $start_time ) ) {
				...
				// Set button label.
				if ( $listing->is_booking_moderated() && $listing->get_id() !== 906) {
				...
				if ( get_option( 'hp_listing_allow_price_extras' ) && $listing->get_price_extras() ) {
					...
					foreach ( $listing->get_price_extras() as $index => $item ) {
						$field_args['options'][ $index ] = [
							...
							'label' => esc_html( sprintf( _x( '%1$s (%2$s)', 'price extra format', 'hivepress-bookings' ), $item['name'], hivepress()->woocommerce->format_price( $item['price'] ) ) ),
						];
